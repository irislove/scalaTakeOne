...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    val resultList = {for((char, charList) <- wordOccursList) yield (char, charList.length)}
    resultList.sorted
  }
...
//        	  if (!wordOccursMap(occurs).isEmpty)
//          } yield combineSentences(wordOccursMap(occurs), generateAnagrams(subtract(occurrences, occurs)))
//    	).flatten
//    }
//    
//    val resultsList = generateAnagrams(sentenceOccurrences(sentence))
//    
//    for {
//      sentenceLeft <- resultsList
//      if sameOccurrences(sentenceOccurrences(sentenceLeft), sentenceOccurrences(sentence))
//    } yield sentenceLeft
//  }
